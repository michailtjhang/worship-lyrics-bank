...
export async function getAllPosts({ includePages = false }) {
  ...
  try {
    ...
    console.error('[getAllPosts] Notion API error (using fallback cache):', err.message || err)
  }

  const postsToFilter = cache || []
  const posts = filterPublishedPosts({ posts: postsToFilter, includePages })
